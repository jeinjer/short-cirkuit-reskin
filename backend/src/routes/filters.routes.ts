...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (category) {
       ...
       if (Object.keys(Category).includes(catUpper)) whereBase.category = catUpper;
    } else {
    ...
    if (minPrice || maxPrice) {
        ...
        if (minPrice) whereBrands.priceUsd.gte = parseFloat(minPrice as string);
        if (maxPrice) whereBrands.priceUsd.lte = parseFloat(maxPrice as string);
    }
