...
const CategorySelector = ({ selectedCategory, onSelectCategory }) => {
  ...
  const renderCategoryButton = (cat, isTodo = false) => {
    ...
    return (
      <button
        ...
        className={`
          group cursor-pointer relative flex items-center justify-center gap-2 sm:gap-3 px-3 sm:px-6 py-3 sm:py-4 transition-all duration-300 border
          rounded-lg w-full
          ${isActive
      ...
      >
        ...
        <span className={`text-xs sm:text-sm md:text-base font-bold tracking-wider font-cyber ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
          {cat.label}
        </span>
      ...
      <div className="container mx-auto px-4">
        <div className="pb-6 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {mainCategories.map((cat) => renderCategoryButton(cat))}
            {allCategory && renderCategoryButton(allCategory, true)}
          </div>
        </div>
      </div>
    </div>
