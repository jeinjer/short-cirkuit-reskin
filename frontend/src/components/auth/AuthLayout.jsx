...
export default function AuthLayout({ children, cardClassName = '', containerClassName = '', initialY = 24 }) {
  return (
    <div className={`min-h-[calc(100dvh-4rem)] bg-[#050507] flex items-start md:items-center justify-center px-4 pt-8 pb-8 md:p-4 relative overflow-hidden ${containerClassName}`}>
      <BackToHomeButton />
      ...
      <motion.div
        ...
        className={`w-full max-w-md bg-[#0b0c12]/95 border border-cyan-500/20 rounded-2xl p-6 md:p-8 shadow-[0_0_40px_rgba(0,0,0,0.55)] relative z-10 backdrop-blur-xl ${cardClassName}`}
      >
        {children}
      </motion.div>
    </div>
