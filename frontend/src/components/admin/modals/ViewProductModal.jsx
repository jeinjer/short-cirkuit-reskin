            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-1/2 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-white/5 p-4 rounded-xl border border-white/5">
                        <div><p className="text-xs text-gray-500 uppercase">Costo</p><p className="text-lg font-mono text-gray-300">${product.costPrice?.toFixed(2)}</p></div>
                        <div><p className="text-xs text-gray-500 uppercase">Venta</p><p className="text-lg font-bold text-green-400">${product.priceUsd?.toFixed(2)}</p></div>
                        <div><p className="text-xs text-gray-500 uppercase">Marca</p><p className="text-white truncate">{product.brand}</p></div>
                        <div><p className="text-xs text-gray-500 uppercase">Categoría</p><p className="text-white truncate">{product.category}</p></div>
                    </div>
                </div>
            </div>
        </div>
