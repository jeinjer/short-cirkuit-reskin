            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
                        ...
                        <th className="p-4 text-right">Venta (USD)</th>
                ...
                <tbody className="divide-y divide-white/5 text-sm">
                    {products.map(p => (
                        <tr key={p.id} className="hover:bg-white/2 transition-colors group">
                            ...
                            <td className="p-4 text-right font-mono text-gray-400">${p.costPrice?.toFixed(2)}</td>
                            <td className="p-4 text-right font-bold text-green-400">${p.priceUsd?.toFixed(2)}</td>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex justify-between items-end mt-2">
                            <div className="flex flex-col">
                                <span className="text-[10px] text-gray-500 uppercase">Venta Público</span>
                                <span className="text-green-400 font-bold text-lg leading-none">${p.priceUsd?.toFixed(2)}</span>
                            </div>
                            <ActionButtons p={p} onView={onView} onEdit={onEdit} />
                        </div>
                    </div>
